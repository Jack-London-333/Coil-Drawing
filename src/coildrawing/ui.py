...
class MainWindow(QMainWindow):
    ...
    def _build_ui(self) -> None:
        ...
        self.in_detail.addItems(["逐匝精细模型（铜线逐匝+分层绝缘+引线）",
                                 "简化束模型（等效整束，生成快）"])
        self.in_leadbr = _dnum(15.0, 2, 200)
        ...
        f4b.addRow("引线错位圆弧半径 (mm)", self.in_leadbr)
        ...
        self.in_seita3 = _dnum(80.0, 0, 90)
        self.in_rd = _dnum(15.0, 1, 100)
        self.in_rd.setToolTip(
            "RD 是内弯半径；圆冠中心线路径半径 Rc=RD+WA/2。"
            "直鼻臂长度 Larm 由 LLM 守恒自动反算，无需输入。")
        ...
        f5.addRow("鼻端中心线与径向直径夹角 seita3 (°)", self.in_seita3)
        f5.addRow("鼻端内弯半径 RD", self.in_rd)
        ...
        f5.addRow("引线轴向直段长 ysc", self.in_ysc)
    ...
    def gather_input(self) -> CoilInput:
        return CoilInput(
            ...
            seita3=math.radians(self.in_seita3.value()), rd_nose=self.in_rd.value(),
    ...
    def apply_input(self, inp: CoilInput) -> None:
        ...
        self.in_f.setValue(inp.f_nose); self.in_seita3.setValue(math.degrees(inp.seita3))
